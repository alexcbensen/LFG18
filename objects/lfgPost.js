const { Discord, EmbedBuilder, WebhookClient, GuildInviteManager } = require("discord.js")
...
LfgPost.prototype.updateStats = function (member, post, client, message, channel) {
    ...
    .then( response => { return response.json().then( data => {
            if (data.status == 200) {
                ...
                let embed = LfgPost.prototype.createMessage(client, channel, message.content, post, post.OP.verified)
...
/*
    if (party.regions.length == 1) {
        embed.addFields({ name: 'Region', value: `${party.region[0]}`, inline: true})
    } else if (party.regions.length > 1) {
        embed.addFields({ name: 'Regions', value: `${party.region}`, inline: true})
    }
*/
    return embed
}
...
LfgPost.prototype.findTags = function (messageArray) {
    if (messageArray.has('mic')) {}
}
